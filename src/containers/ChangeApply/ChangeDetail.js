...
export default class ChangeDetail extends Component{
    constructor(props){
        ...
        const replyInfo = JSON.parse(cache.getItem('commInfo')).info;
        this.state={
    ...
    getCaseInfo=()=>{
        ...
        if(!caseInfo){
            ...
            if(changeId){
                ajax.get(`/litigant/case/${changeId}/changeDoc`,{
                    params:{
                        caseId
                    }
                }).then((data)=>{
                ...
                ajax.get(`/litigant/case/${caseId}/caseInfo`).then((data)=>{
                    const {baseInfo,payInfo}=data;
                    this.setInfo(baseInfo,payInfo,this.getDateList(payInfo));
                }).finally(()=>{
                    this.setState({
    ...
    getContent=()=>{
        ...
        return(<Spin spinning={spinning}  tip="加载中...">
            ...
            {caseInfo && <Book className={this.state.messageH} getFrameHeight={this.getFrameHeight.bind(this)} title={title} caseInfo={caseInfo} type={0}></Book>}
        </Spin>)
    }
    toggle=()=>{
    ...
    goReply=()=>{
        ...
        ajax({
            url:`litigant/case/${caseId}/replyDoc`,
            method:'get',
            params:{
                caseId,
                type
            ...
            history.push(`${location.pathname}/reply/${type}`);
        })
    }
    checkReply=()=>{
        const {location,history,baseInfo:{changeId},match:{params:{caseId}}}=this.props;
    ...
    render(){
        ...
        return(<section className="changeDetail">
            ...
            <Modal className="preview-modal"
                ...
                <ArbitrationPreview info={replyInfo} type="thisReply"/>
            </Modal>
            <Modal
                visible={standard}
                title="收费标准"
