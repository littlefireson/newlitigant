...
export default class Jurisdiction extends Component{
    ...
    componentDidMount(){
        ...
        ajax.get(`/litigant/case/${caseId}/getBackJurisInfo`).then((data)=>{
            ...
            if(feedbackDocVO){
                feedbackDocVO.type=4;
                feedbackDocVO.requestItem = feedbackDocVO.requset;
                feedbackDocVO.factReason = feedbackDocVO.reasonFact;
                requestList.push(feedbackDocVO);
            }
            if(decisionDocVO){
                decisionDocVO.type=11;
    ...
    getList=()=>{
        const {requestList,spinning,feedbackBtn} =this.state;
